>      __    _____ _____ __   __ __   __   ______ _____
>     |  |  |  _  |  _  |  | |  |  | |  | |__  __|  ___|
>     |  |  | | | | |_| |  | |  |  | |  |    | | | |___
>     |  |__| |_| |___  |  |_|  |  |_|  |   _| | |____ |
>     |_____|_|_|_|___|_|_______|_______|[]|___| |_____|


** 注意! **

** 次回予定のバージョンアップで、現在laquu.jsで利用している「$.laquu.関数名」は利用できなくなります！ **

** 次回移行は$("selector").関数名({オプションオブジェクト});のような従来jQueryで推奨されているプラグインの記述形式へと統一されます。 **

*** アップデート時期はまだ未定ですが、10月いっぱいまでには完全に移行出きるよう準備しております。 **
